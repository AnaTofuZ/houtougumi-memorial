...
- ダミーコンテンツと依存関係の脆弱性が残っているため、現時点では公開不可
...
- [ ] サイトの対象を「宝灯桃汁」単独にするか「ほうとう組」全体にするか確定し、名称と説明を統一する
...
- [ ] 権利者からの問い合わせ・削除依頼を受ける連絡先を用意する
- [ ] 非公式サイトであることを各種メタ情報を含めて明示する

### ビルド・依存関係

- [ ] `@astrojs/check` と `typescript` を導入する
- [ ] `npm run check` スクリプトを追加する
- [ ] `npm audit fix` を実行する
- [ ] high脆弱性4件（`js-yaml`、`nanoid`、`sharp`、`svgo`）の解消を確認する
- [ ] 更新後に型チェック、ビルド、依存関係監査を再実行する
...
- [ ] 正式なfaviconを用意する
- [ ] `robots.txt` を追加し、本番環境だけインデックスを許可する
- [ ] 必要に応じてサイトマップを追加する

### エラーページ・画像・外部通信

- [ ] `src/pages/404.astro` を追加する
- [ ] 画像へ `width` と `height` を設定する
- [ ] 画面外画像へ `loading="lazy"` を設定する
- [ ] 正式画像をWebPまたはAVIFなどへ最適化する
- [ ] Google FontsとYouTube埋め込みによる外部通信を確認する
- [ ] 必要に応じてプライバシー案内を掲載する

### アクセシビリティ

- [ ] モバイルメニューに `aria-expanded` を追加し、表示状態と同期する
- [ ] デスクトップのメンバーメニューをキーボード操作可能にする
- [ ] ファンアート一覧のクリック可能な `div` を `button` に変更する
- [ ] ライトボックスへdialog属性、Escで閉じる操作、フォーカス制御を追加する
- [ ] グッズ絞り込みボタンへ選択状態を付与する
